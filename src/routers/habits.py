...
async def create_habit(habit_in: HabitCreate, current_user: TokenData = Depends(get_current_user)):
    new_habit = Habit(
...
async def delete_habit(habit_id: str, current_user: TokenData = Depends(get_current_user)):
    # Requiere rol admin
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Permisos insuficientes: se requiere admin")
    habit = await Habit.get(habit_id)
    if not habit:
        raise HTTPException(status_code=404, detail="Hábito no encontrado")
    await habit.delete()
