...
async def create_daily_log(log_in: DailyHabitLogCreate, current_user: TokenData = Depends(get_current_user)):
    habit = await Habit.get(log_in.habit_id)
    if not habit or habit.owner_id != current_user.user_id:
        raise HTTPException(status_code=404, detail="Hábito no encontrado o no perteneciente al usuario")
    log = DailyHabitLog(
...
async def get_my_daily_logs(current_user: TokenData = Depends(get_current_user)):
    logs = await DailyHabitLog.find(DailyHabitLog.user_id == current_user.user_id).to_list()
...
async def delete_daily_log(log_id: str, current_user: TokenData = Depends(get_current_user)):
    log = await DailyHabitLog.get(log_id)
    if not log:
        raise HTTPException(status_code=404, detail="Registro diario no encontrado")
    if log.user_id != current_user.user_id:
